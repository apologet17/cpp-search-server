...
ostream& operator<<(ostream& os, const vector<string_view>& inputVect) {
    os << "["s;
    bool first = true;
...
void TestMatchedWords() {
    ...
    //Поисковый запрос совпадает с содержимым документа - вернуть все слова
    {
        ...
        const auto wordsFromQuery_par = server.MatchDocument(std::execution::par, content[0], doc_id[0]);
        auto wordsFromContent = SplitIntoWordsSV(content[0]);
    ...
    {
        auto qery = "cat in the city out"s;
        const auto wordsFromQuery = server.MatchDocument(qery, doc_id[0]);
        auto wordsFromContent = SplitIntoWordsSV(qery);
        const auto wordsFromQuery_par = server.MatchDocument(std::execution::par, qery, doc_id[0]);
        sort(wordsFromContent.begin(), wordsFromContent.end());
     //   ASSERT_EQUAL(wordsFromContent, get<0>(wordsFromQuery));
     //   ASSERT_EQUAL(wordsFromQuery_par, get<0>(wordsFromQuery));
        ASSERT_EQUAL(DocumentStatus::ACTUAL, get<1>(wordsFromQuery));
    }

    //Поисковый запрос содержит меньше слов, чем есть в подходящем документе - вернуть только слова запроса
    {
        auto qery = "cat in the"s;
        const auto wordsFromQuery = server.MatchDocument(qery, doc_id[0]);
        const auto wordsFromQuery_par = server.MatchDocument(std::execution::par, qery, doc_id[0]);
        auto wordsFromContent = SplitIntoWordsSV(qery);
        sort(wordsFromContent.begin(), wordsFromContent.end());
...
void TestRelevanceSort() {
    ...
    const vector<string> content = { "ui ui ui ui"s, "cat dog fat rat"s, "cat ty asas hytr"s, "re fd asas hytr"s };
    const vector<int> ratings = { 1, 2, 3 };
